...
// i18n language subdirectories generated by generate-i18n-routes.js
const I18N_LANG_DIRS = new Set(['en', 'es', 'pt', 'it', 'de', 'ar', 'nl']);

function getFilesRecursively(dir, extension, filesList = [], isRoot = true) {
  if (!fs.existsSync(dir)) return filesList;
  const files = fs.readdirSync(dir);
  for (const file of files) {
    const filePath = path.join(dir, file);
    if (fs.statSync(filePath).isDirectory()) {
      // Skip node_modules, .astro, AND the generated i18n language subdirs at root level
      if (file === 'node_modules' || file === '.astro') continue;
      if (isRoot && I18N_LANG_DIRS.has(file)) {
        console.log(`[i18n] Skipping validation of generated language directory: dist/${file}/`);
        continue;
      }
      getFilesRecursively(filePath, extension, filesList, false);
...
function checkLocalLink(currentFilePath, linkUrl) {
  ...
  // 3. Skip links that point into i18n language subdirectories (e.g. /en/page, /es/guide)
  //    These are generated by generate-i18n-routes.js and valid at deploy time
  if (urlWithoutHash.startsWith('/')) {
    const firstSegment = urlWithoutHash.split('/').filter(Boolean)[0];
    if (firstSegment && I18N_LANG_DIRS.has(firstSegment)) {
      return true;
    }
  }

  // 4. Resolve absolute vs relative
  let targetPath;
  if (urlWithoutHash.startsWith('/')) {
    // Relative to distDir
  ...
  // 5. Check if the path exists in any of the common formats:
